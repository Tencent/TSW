...
const {intranetIp} 		= require('serverInfo.js');
const {debugOptions} 	= process.binding('config');
const methodMap			= {};
const workerMap			= {};
const cpuMap			= [];
const timePos			= parseInt(String(intranetIp).slice(-1),10) || 0;   //时间偏移
var isDeaded			= false;
...
function startServer(){
	...
	if(cluster.isMaster && useWorker){
		...
		process.on('heapdump',function(GET){
			sendCmd2workerOnce('heapdump',GET);
		});
...
function restartServer(){
	...
	logger.info('restart server by pid: ${pid}',{
		pid: process.pid
	});

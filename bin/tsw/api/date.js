...
const Date = {
    ...
    format: function(mDate, fmt) {
        ...
        if (/(Y+)/.test(fmt)) {
            fmt = fmt.replace(RegExp.$1, (String(mDate.getFullYear())).substr(4 - RegExp.$1.length));
        }
        if (/(E+)/.test(fmt)) {
            fmt = fmt.replace(RegExp.$1, ((RegExp.$1.length > 1) ? (RegExp.$1.length > 2 ? '\u661f\u671f' : '\u5468') : '') + week[String(mDate.getDay())]);
        }
